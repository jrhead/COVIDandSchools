#### Functions needed in the Agent Based Model Simulation ####
##
## Functions include:
...
#  4. SEIR_func: runs the SEIR model, assuming symptomatic individuals do not isolate
#  5. modelled_outcomes: counts/summarizes important outcomes (e.g. cumulative infections, daily hospitalizations)
##

# Function to define the fates for N people and the waiting times between each of their stages #
...
fate_func <- function(N, #Number of people in population
  ...
  # determine probability of each fate
  fate <- rep(NA,N)
  A_prob <- 1-rho
  C_prob <- rho*(1-h_rate)
  ...
  waitEI <- rweibull(N, shapeEI, scaleEI) #wait between exposed and (infectious)
  waitIR <- rweibull(N, shapeIR, scaleIR) #wait between infectious and recovered
  waitIH <- rweibull(N, shapeIH, scaleIH) #wait between infectious and hospitalized
  waitHRD <- rweibull(N, shapeHRD, scaleHRD) #wait between hospitalization and death/recovery
...
# Function to create and return an N x N community matrices for various school/counterfactual scenarios
...
SEIRQ_func <- function(synth_pop_df, #synthetic population
                      ...
                      #superSpreader, #vector of who is a superspreader
  ...
  for (day in start_day:end_day){
    ...
    SymptVect <- SymptVectWRKSCH <- SymptVectCOM <- as.numeric(state %in% c("C", "H1", "D1"))
    if(day >= 3){ #only isolate on 3rd day of being infectious
      ...
      qID <- as.numeric(as.character(qID))
        ...
        if(length(complyCase>0)){
          ...
          SymptVectWRKSCH[complyCase] <- 0 #reduce school and work contacts by 100%
    ...
    SymptContVect <- contacts_orig_list[[1]] %*% SymptVect +
      contacts_orig_list[[2]] %*% SymptVectWRKSCH +
      contacts_orig_list[[3]] %*% SymptVectWRKSCH +
      contacts_orig_list[[4]] %*% SymptVectWRKSCH +
      contacts_orig_list[[5]] %*% SymptVectWRKSCH +
      contacts_orig_list[[6]] %*% SymptVectCOM
    # Determine contact with asymptomatic individuals
    AsymptContVect <- contacts_orig_list[[1]] %*% as.numeric(state == "A") +
      contacts_orig_list[[2]] %*% as.numeric(state == "A") +
      contacts_orig_list[[3]] %*% as.numeric(state == "A") +
      contacts_orig_list[[4]] %*% as.numeric(state == "A") +
      contacts_orig_list[[5]] %*% as.numeric(state == "A") +
      contacts_orig_list[[6]] %*% as.numeric(state == "A")
...
# Function to run the SEIR with no isolation of symptomatic cases

SEIR_func <- function(synth_pop_df, #synthetic population
                       start_day, #initial day of simulation
                       end_day, #last day of simulation
                       contacts_orig_list, #original contact matrices broken up by household, school, workplace, community, grade and class
                       contacts, #single matix of list added together
                       alpha, #ratio of infected cases transmitting to non-infected
                       beta, #scaling parameter
                       time_state1, time_next_state1, #distribution of waiting times to get to first state
                       time_state2, time_next_state2, #distribution of waiting times to get to second state
  ...
  for (day in start_day:end_day){
    ### 1. Movement of S to E 
    # Determine contact with symptomatic individuals
    SymptContVect <- contacts_update_list[[1]] %*% as.numeric(state %in% c("C", "H1", "D1")) +
      contacts_update_list[[2]] %*% as.numeric(state %in% c("C", "H1", "D1")) +
      contacts_update_list[[3]] %*% as.numeric(state %in% c("C", "H1", "D1")) +
      contacts_update_list[[4]] %*% as.numeric(state %in% c("C", "H1", "D1")) +
      contacts_update_list[[5]] %*% as.numeric(state %in% c("C", "H1", "D1")) +
      contacts_update_list[[6]] %*% as.numeric(state %in% c("C", "H1", "D1"))
    # Determine contact with asymptomatic individuals
    AsymptContVect <- contacts_update_list[[1]] %*% as.numeric(state == "A") +
      contacts_update_list[[2]] %*% as.numeric(state == "A") +
      contacts_update_list[[3]] %*% as.numeric(state == "A") +
      contacts_update_list[[4]] %*% as.numeric(state == "A") +
      contacts_update_list[[5]] %*% as.numeric(state == "A") +
      contacts_update_list[[6]] %*% as.numeric(state == "A")
...
modelled_outcomes <- function(state_full,#output of SEIR function - dataframe of states per agent per day
  ...
  # subset dataframe to those households
  ...
  res <- list("TimeSeries_I" = TimeSeries_I, "TimeSeries_IC" = TimeSeries_IC, 
              "TimeSeries_M" = TimeSeries_M, "TimeSeries_Hosp" = TimeSeries_Hosp,
              "CumTotal_I" = CumTotal_I, "CumTotal_IC" = CumTotal_IC, 
              "CumTotal_M" = CumTotal_M, "CumSchoolHH_I" = CumSchoolHH_I, 
              "CumSchoolHH_IC" = CumSchoolHH_IC, "CumSchoolHH_M" = CumSchoolHH_M,
