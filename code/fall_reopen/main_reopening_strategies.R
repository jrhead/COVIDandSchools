...
outcomes <- foreach(reps = 1:1000, .packages = "dplyr") %dopar% {
  ...
  #Call SEIR function
  StatesPhase3_OPEN <- SEIRQ_func(synth_pop_df, 
  ...
  state_full.OPEN <- StatesPhase3_OPEN[["state_full"]]
  ...
  #Update contact list to reflect students come 2 days per week in half classes
  ...
  #Update contact matrix to refect 75% reduction in non-classroom contacts 
  # + 2 days per week half class size shifts
  S3_contact_list <- contacts_orig_list
  S3_contact_list[[2]] <- MidEssentialWorkMatsp
  S3_contact_list[[3]] <- (2/5)*0.25*S3_contact_list[[7]] #School shifts; 75% reduction
  S3_contact_list[[4]] <- (2/5)*0.25*S3_contact_list[[8]] #Grade shifts; 75% reduction
  S3_contact_list[[5]] <- (2/5)*(5/7 - 0.25*1/7 - 0.25*.2/7)/(5/7 - 1/7 - 0.2/7)*S3_contact_list[[9]] #Class
  ...
  ################## Sim1: Test teachers weekly ############################
  
  #Update contact matrix: no changes to OPEN because only intervention is testing
  OPEN_contact_list <- contacts_orig_list
  OPEN_contact_list[[2]] <- MidEssentialWorkMatsp
  OPEN_contact_list[[6]] <- gen_COMmat(CommContactIncr*Obs.comm) #Community, EXACTLY as observed, with work, daycare, public trans included
  
  #Call SEIR function that permits testing
  StatesPhase3_TT7 <- SEIRtest_func(synth_pop_df, 
                                    start_day = 213, #Aug 15
                                    end_day = 340, #Dec 20
                                    test_day = 7, #test every 7 days
                                    test_students = 0, #only test teachers
                                    contacts_orig_list = OPEN_contact_list, 
                                    alpha,
                                    beta = fates[["beta"]],
                                    time_state1 = fates[["time_state1"]], time_next_state1 = StatesPhase2[["time_next_state1"]],
  ...
  state_full.TT7 <- StatesPhase3_TT7[["state_full"]]
  
  #extract summary indicators
  outcomeTT7 <- modelled_outcomes(state_full.TT7, synth_pop_df)
  
  rm(state_full.TT7);
  gc()
  
  ################## Sim2: test teachers and students weekly ############################

  #Call SEIR function that permits testing
  StatesPhase3_TB7 <- SEIRtest_func(synth_pop_df, 
                                    start_day = 213, #Aug 15
                                    end_day = 340, #Dec 20 
                                    test_day = 7, #test every 7 days
  ...
  state_full.TB7 <- StatesPhase3_TB7[["state_full"]]
  ...
  ################## Sim3: test teachers monthly ############################
  
  #Call SEIR function that permits testing
  StatesPhase3_TT30 <- SEIRtest_func(synth_pop_df, 
                                    start_day = 213, #Aug 15
                                    end_day = 340, #Dec 20 
  ...
  state_full.TT30 <- StatesPhase3_TT30[["state_full"]]
  
  #extract summary indicators
  outcomeTT30 <- modelled_outcomes(state_full.TT30, synth_pop_df)
  
  rm(state_full.TT30);
  gc()
  
  ################## Sim4: test both teachers & students monthly ############################
  
  StatesPhase3_TB30 <- SEIRtest_func(synth_pop_df, 
                                     start_day = 213, #Aug 15
                                     end_day = 340, #Dec 20
                                     test_day = 30, #test every 30 days
                                     test_students = 1, #do test students
  ...
  state_full.TB30 <- StatesPhase3_TB30[["state_full"]]
  
  #extract summary indicators
  outcomeTB30 <- modelled_outcomes(state_full.TB30, synth_pop_df)
  ...
  StatesPhase3_MASK <- SEIRQ_func(synth_pop_df, 
                                  start_day = 213, #Aug 15
                                  end_day = 340, #Dec 20
                                  contacts_orig_list = OPEN_contact_list, 
                                  alpha,
  ...
  state_full.MASK <- StatesPhase3_MASK[["state_full"]]
  ...
  ################## Sim6: Mask, monthly testing, class groups ############################
  
  #Update contact matrices to reflect reductions in non-classroom contacts
  S1_contact_list <- contacts_orig_list

  S1_contact_list[[2]] <- MidEssentialWorkMatsp
  S1_contact_list[[3]] <- 0.25*S1_contact_list[[3]] #reduce school-only contacts
  S1_contact_list[[4]] <- 0.25*S1_contact_list[[4]] #reduce grade-only contacts
  S1_contact_list[[5]] <- (5/7 - 0.25*1/7 - 0.25*.2/7)/(5/7 - 1/7 - 0.2/7)*S1_contact_list[[5]]
  S1_contact_list[[6]] <- gen_COMmat(CommContactIncr*Obs.comm) #Community, EXACTLY as observed, with work, daycare, public trans included

  #call SEIR function that allows for testing
  StatesPhase3_M.TT30.S1 <- SEIRtest_func(synth_pop_df, 
                                         start_day = 213, #Aug 15
                                         end_day = 340, #Dec 20
                                         test_day = 30, #test every 30 days
                                         test_students = 0, #dont test students
                                         contacts_orig_list = S1_contact_list, 
                                         alpha,
                                         beta = fates[["beta"]],
                                         time_state1 = fates[["time_state1"]], time_next_state1 = StatesPhase2[["time_next_state1"]],
                                         ...
                                         propComplyHH = 0.25,
                                         redContacts = 0.75,
                                         mask_ef_teacher = 0.5, #do wear masks
                                         mask_ef_elem = 0.35,
                                         mask_ef_middle = 0.25,
                                         mask_ef_HS = 0.15)
