...
# including potential membership with staggered half-class shifts
...
synth_pop <- function(N, #number of agents
                      ...
                      NShifts, #number of division of class groups
  ...
  numHH <- length(unique(HH))
  
  ## 4. Assign ages based on HH ids
  
  #get ids for the different types of HH's
  HH_nokids <- c(1:max(HH_nf))
  HH_kids <- c(max(HH_nf)+1:max(HH_f))
  ...
  #Create and determine vector of continuous ages
  ...
  # Multimodal dist for nonparents: young and older adults more likely to not be parents
  ...
  #check that proportions are more or less okay
  # sum(Age <= 5)/N
  # sum(Age > 5 & Age < 18)/N
  # sum(Age < 18)/N
  # sum(Age > 65)/N
  ...
  ##9. Specify who would come in which shift during a staggered 2 day schedule by grade
  Shift <- rep(NA,N)
  
  #Elementary schools: grades 0-2 come in one shift, 3-5 in another
  Shift[School < 1000 & Grade %in% c(0,1,2)] <- 1 
  Shift[School < 1000 & Grade %in% c(3,4,5)] <- 2
  
  #Middle schools: grades 6-7 come in one shift, 8 in another
  Shift[School >= 1000 & School < 100000 & Grade %in% c(6,7)] <- 1
  Shift[School >= 1000 & School < 100000 & Grade %in% c(8)] <- 2
  
  #Hig schools: grades 9-10 come in one shift, 11-12 in another
  Shift[School >= 100000 & Grade %in% c(9,10)] <- 1
  Shift[School >= 100000 & Grade %in% c(11,12)] <- 2
  
  #Admin (assigned school only, not grade or class) come to school for all 4 days
  #Teachers (assigned schools, grades, and class) come to school with class (2 days)
  Shift[is.na(Shift) & !is.na(School) & is.na(Class)] <- 0
  Shift[is.na(Shift) & !is.na(School)& is.na(Class)] <- 0
  
  ## 10. Create dataframe of synthetic population and return
  synth_pop <- data.frame(cbind(ID, Age, AgeCat, Community, HH, School, Grade, Class, Shift, Work))
  return(synth_pop)
