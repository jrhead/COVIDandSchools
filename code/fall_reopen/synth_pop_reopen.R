...
synth_pop <- function(N, #number of agents
 ...
 #Draw from multimodel distribution, with children spaced on average 2 years apart
  ...
  #Specify who would come in morning or afternoon in a split day or week situation...
  Shift <- rep(NA,N)
  SchoolGradeClassVect <- unique(SchoolGradeClass[!is.na(SchoolGradeClass)])
  for (i in 1:length(SchoolGradeClassVect)){
    Shift[!is.na(SchoolGradeClass) & SchoolGradeClass == SchoolGradeClassVect[i]] <- 
      sample(1:NShifts, sum(SchoolGradeClass == SchoolGradeClassVect[i], na.rm = T), replace = TRUE)
  }
  ...
  #Admin and teachers stay at school all day during shifts
  Shift[is.na(Shift) & !is.na(School)] <- 0
  Shift[is.na(Shift) & !is.na(School)] <- 0
